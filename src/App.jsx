    ...
    <div className="p-6 w-full min-h-screen bg-gradient-to-b from-zinc-900 to-zinc-800 flex items-center justify-center">
      <div className="flex items-center justify-center flex-col gap-4 w-full max-w-6xl">
       <h1 className="font-bold mb-8 text-4xl text-transparent bg-clip-text bg-gradient-to-r from-yellow-400 via-amber-500 to-rose-400">
          ...
          <div className="mb-8 flex width-full max-w-lg shadow-lg rounded-lg overflow-hidden">
            <input 
              ...
              >
                {Object.keys(columns).map((columnId) => (
                  ...
                  <button onClick={addNewTask} className="p-3 bg-gradient-to-r from-yellow-600 to-amber-500 text-white font-medium
                  hover:from-yellow-500 hover:to-amber-400 transition-all duration-200 cursor-pointer">
                    Add 
                    </button>
            ...
            >
              ...
              {columns[columnId].items.length === 0 ? (
                ...
                columns[columnId].items.map((item) => (
                  <div key={item.id} className='p-4 mb-3 bg-zinc-700 text-white rounded-md shadow-md cursor-move flex items-center justify-between transform transition-all duration-200 hover:scale-105 hover:shadow-lg' draggable onDragStart={() => handleDragStart(columnId, item)}>
                    <span className='mr-2'>{item.content}</span>
                    <button onClick={() => removeTask(columnId, item.id)} className='text-zinc-400 hover:text-red-400 transition-colors duration-200 w-6 h-6 flex items-center justify-center rouded-full hover:bg-zinc-600'>
